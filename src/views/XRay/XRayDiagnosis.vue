<template>
  <div class="mask-opacity diagnosis-results overflow-scroll no-scrollbar flex flex-col">
    <!-- Conclusiones -->
    <DiagnosisCard title="Conclusiones">
    ...
    <!-- Recomendaciones -->
    <DiagnosisCard title="Recomendaciones">
      <template v-for="(suggestion, index) in suggestions" :key="index">
        <div class="flex mb-6"> 
          ...
          >
            ...
            <svg v-if="suggestion.importance == 'red'" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="http://www.w3.org/2000/svg" width="24px"
            ...
            <svg v-else xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="http://www.w3.org/2000/svg" width="24px"
    ...
    <DiagnosisCard v-if="messages && messages && messages.length > 0" title="Chat con Asistente IA">
  ...
  <form @submit.prevent="sendUserMessage" class="absolute bottom-0 w-full">
    <div class="relative">
      <input 
        ...
        type="text" 
        placeholder="Haga una pregunta sobre el diagnóstico..."
        :disabled="isLoading"
      />
      <button 
        type="submit"
    ...
    <p v-if="isLoading" class="text-xs text-center mt-1">Procesando su consulta...</p>
...
export default {
  ...
  computed: {
    conclusions() {
      return this.openAIStore.conclusions || this.conclusionesEjemplo;
    },
    suggestions() {
      return this.openAIStore.suggestions || this.sugerenciasEjemplo;
    },
    messages() {
  ...
  methods: {
    async sendUserMessage() {
      ...
      // Si aún no hay un hilo activo, crear un nuevo chat
      if (!this.openAIStore.threadId) {
        await this.openAIStore.createChat(this.userInput);
      } else {
        // Si no, enviar mensaje al hilo existente
